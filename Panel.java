...
public class Panel extends JPanel implements ActionListener {       //GamePanel class extends properties of JPanel class and implements properties of interface ActionListner also
    ...
    int snake_length = 4;                             //initially set blocks for snake body
    int SCORE = 0;                                     //food items eaten by snake initially set to 0
    ...
    { //we take a parameter g from Graphics class uswd in drawing the graphical structure in java
    ...
    public void draw(Graphics g)
    {
        ...
        if (running) {  //this method first checks for runnning status of our game that if it is true then we can move inside the if block
            ...
            g.drawString("Score: " + SCORE, (SCREEN_WIDTH - metrics.stringWidth("Score: " + SCORE)) / 2, g.getFont().getSize()); //drawString method draw the String on our panel by taking the following parameters("String that we want to draw ",x-length,y-length)
        } else {
    ...
    public void newFood() {
        snakeFoodX = random.nextInt((SCREEN_HEIGHT / UNIT_SIZE))* UNIT_SIZE; //STORES X CO-ORDIATE OF FOOD
        snakeFoodY = random.nextInt((SCREEN_WIDTH / UNIT_SIZE)) *UNIT_SIZE; //STORES Y CO-ORDIATE OF FOOD
    ...
    public void move()
    {
        ...
        { //starting from snake length it continusoly shifts the values of x and y axis which are arrays where we store the co-ordinates of snake
            x[i] = x[i - 1];
            y[i] = y[i - 1];
        }
    ...
    public void EAT()
    {
        ...
        { //here as x and y array includes the dimensins of our snake therefore if dimension of head of our snake is equal to the position of food then it must eat it
            snake_length++; //now as our snake eat it's food then we increase snake's length by 1
            SCORE++; // and also increase the value of food eaten by our snake to one  that will help further in showing score
    ...
    public void checkCollisions()
    {
        ...
        if (!running)
        { //check if running is not true then stop timer
            timer.stop();
        }
    }
    ...
    public void actionPerformed(ActionEvent e) { //now overriding the method actionPerformed and taking a parameter e declared through actionEvent class that fetch response as after we press buttons while playing game
        ...
        if (running) { //if game is running then
            ...
            EAT(); // calling checkFood function
